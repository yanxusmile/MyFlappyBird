...
void Pipe::SpawnPipe(cocos2d::Layer *layer)
{
    ...
    //set collision bitmask for pipes
    ...
    //create pipe movement action
    auto topPipAction = MoveBy::create(PIPE_MOVEMENT_SPEED * visibleSize.width, Point( - visibleSize.width * 1.5, 0) );
    auto bottomPipAction = MoveBy::create(PIPE_MOVEMENT_SPEED * visibleSize.width, Point( - visibleSize.width * 1.5, 0) );

    topPipe->runAction(topPipAction);
    bottomPipe->runAction(bottomPipAction);
    
    //create node and its physics body for gaining points
